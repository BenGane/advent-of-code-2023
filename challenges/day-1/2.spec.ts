...
it("works", async () => {
  ...
  const totalCalibration = lines.reduce((accumulator, line) => {
    const firstNumbersFound = [...numbers]
      .map((number) => ({ index: line.indexOf(number), number }))
      .filter(({ index }) => index !== -1);
    const lastNumbersFound = [...numbers]
      .map((number) => ({ index: line.lastIndexOf(number), number }))
      .filter(({ index }) => index !== -1);

    firstNumbersFound.sort((a, b) => a.index - b.index);
    lastNumbersFound.sort((a, b) => a.index - b.index);

    const firstWordsFound = words
      .map((word) => ({ index: line.indexOf(word), word }))
      .filter(({ index }) => index !== -1);
    const lastWordsFound = words
    ...
    const lastWord = lastWordsFound[lastWordsFound.length - 1] ?? {
      index: -Infinity,
    };

    const firstNumber = firstNumbersFound[0] ?? { index: Infinity };
    const lastNumber = lastNumbersFound[lastNumbersFound.length - 1] ?? {
      index: -Infinity,
    };
